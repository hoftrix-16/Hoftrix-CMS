...
const ProtectedRoute = ({ route, children }: { route: any; children: React.ReactNode }) => {
  ...
  if (user && user.role !== 'admin') {
    // 1. Always allow profile, calendar, fallback, root, sign-in, etc.
    // Calendar is always shown in employee sidebar — keep route access in sync.
    ...
    if (matchingKey) {
      ...
      const hasPermission =
        matchingKey === 'mgmt-calendar' ||
        matchingKey === 'page-profile' ||
